...
-(id)initWithCustomers:(NSMutableArray*)myCustomers andUser:(CSUser*)myUser{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        nibName = @"CSCustomerListViewController_Ipad";
    }else{
        nibName = @"CSCustomerListViewController";
    }
...
- (NSString *)getPngOfCustomer:(CSCustomer *)cust {
    if ([cust.other isEqualToString:@"03"])
    {
        if ([cust.srvy isEqualToString:@"X"])
        {
            return @"surveyKapaliBig.png";
        }
        else
        {
            return @"greenCustomerIconBig.png";
        }
    }
...
{    
    ...
    for (int i = 0; i < [customers count]; i++) {
        CSCustomer *tempCustomer = [customers objectAtIndex:i];
        NSString *kunnr = tempCustomer.kunnr;
        
        // Skip leading zeros
        NSScanner *scanner = [NSScanner scannerWithString:kunnr];
        ...
        NSString *result = [kunnr substringFromIndex:[scanner scanLocation]];
        
        NSComparisonResult result2 = [result compare:searchText options:(NSCaseInsensitiveSearch|NSDiacriticInsensitiveSearch) range:NSMakeRange(0, [searchText length])];
