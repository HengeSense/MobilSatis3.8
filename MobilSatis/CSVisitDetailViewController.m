...
-(void)getResponseWithString:(NSString *)myResponse andSender:(ABHSAPHandler *)me {
    ...
    @try {
        if ([[responses objectAtIndex:0 ] isEqualToString: @"T"]) {
            alert = [[UIAlertView alloc] initWithTitle:@"Başarılı" message:[NSString stringWithFormat:@"%@ müşterisine ziyaret girişi başarıyla yapılmıştır.",customer.name1] delegate:self cancelButtonTitle:@"Tamam" otherButtonTitles: nil];
            alert.tag = 5;
        ...
        else if([[responses objectAtIndex:0 ] isEqualToString: @"F"])
        {
            NSMutableArray *responses = [ABHXMLHelper getValuesWithTag:@"E_MESSAGE" fromEnvelope:myResponse];
            NSString *err = [responses objectAtIndex:0];
            alert = [[UIAlertView alloc] initWithTitle:@"Hata" message:[NSString stringWithFormat:@"%@",err] delegate:self cancelButtonTitle:@"Tamam" otherButtonTitles: nil];
            alert.tag = 7;
