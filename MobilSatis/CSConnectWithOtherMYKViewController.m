...
- (void)initOtherMYKListFromEnvelope:(NSString *)envelope {
    @try {
        ...
        description = [ABHXMLHelper getValuesWithTag:@"STEXT" fromEnvelope:envelope];
        ...
        for (int i = 0; i < [objectCount count]; i++) {
            ...
            if ([[sales_office objectAtIndex:i] isEqualToString:@""]) {
                [otherMYK setSales_office:@"Merkez"];
            }
            else
            {
            [otherMYK setSales_office:[description objectAtIndex:i]];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (countNumber == 0) {
        ...
        //        lastMYK = myk;
        CSOtherMYK *mykBuffer = [[CSOtherMYK alloc] init];
        [otherMYKTableList insertObject:mykBuffer atIndex:0];
    }
    else
    {
        countNumber--;
