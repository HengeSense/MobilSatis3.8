...
- (id)initWithUser:(CSUser *)myUser
{
    ...
    {
        self = [super initWithNibName:@"CSUserSalesViewController_Ipad" bundle:nil];
    }
...
-(void)getResponseWithString:(NSString *)myResponse andSender:(ABHSAPHandler *)me{
    ...
    if ([[me RFCNameLine] rangeOfString:@"ZMOB_MYK_AYLIKKUMULE" options:NSCaseInsensitiveSearch].location != NSNotFound)
    {
        int sayac = 0;
        chartHeader1 = @"";
        chartHeader2 = @"";
        ...
        if ([[[ABHXMLHelper getValuesWithTag:@"TABLO1_GOSTER" fromEnvelope:myResponse] objectAtIndex:0] isEqualToString:@"X"]) {
            chartHeader1 = [NSString stringWithFormat:@"%@",[[ABHXMLHelper getValuesWithTag:@"TABLO1_BASLIK" fromEnvelope:myResponse] objectAtIndex:0] ];
            chartReport1Texts = [[NSMutableArray alloc] init];
            chartReport1 = [[NSMutableArray alloc] init];
            [self parseResponse:[[ABHXMLHelper getValuesWithTag:@"ZMOB_AYLIK_LITRE" fromEnvelope:myResponse] objectAtIndex:sayac] intoArray:&chartReport1 andTexts:&chartReport1Texts];
            sayac++;
        }
        if ([[[ABHXMLHelper getValuesWithTag:@"TABLO2_GOSTER" fromEnvelope:myResponse] objectAtIndex:0] isEqualToString:@"X"]) {
            chartHeader2 = [[ABHXMLHelper getValuesWithTag:@"TABLO2_BASLIK" fromEnvelope:myResponse] objectAtIndex:0];
            chartReport2Texts = [[NSMutableArray alloc] init];
            chartReport2 = [[NSMutableArray alloc] init];
            [self parseResponse:[[ABHXMLHelper getValuesWithTag:@"ZMOB_AYLIK_LITRE" fromEnvelope:myResponse] objectAtIndex:sayac] intoArray:&chartReport2 andTexts:&chartReport2Texts];
            sayac++;
        }
        if ([[[ABHXMLHelper getValuesWithTag:@"TABLO3_GOSTER" fromEnvelope:myResponse] objectAtIndex:0] isEqualToString:@"X"]) {
            chartHeader3 = [[ABHXMLHelper getValuesWithTag:@"TABLO3_BASLIK" fromEnvelope:myResponse] objectAtIndex:0];
            chartReport3Texts = [[NSMutableArray alloc] init];
            chartReport3 = [[NSMutableArray alloc] init];
            [self parseResponse:[[ABHXMLHelper getValuesWithTag:@"ZMOB_AYLIK_LITRE" fromEnvelope:myResponse] objectAtIndex:2] intoArray:&chartReport3 andTexts:&chartReport3Texts];
        }
        ...
        [mv.navigationController setToolbarHidden:YES];
        [mv.view setClipsToBounds:YES];
        [mv refresh];
...
-(void)parseResponse:(NSString*)response intoArray:(NSMutableArray*__strong *)array andTexts:(NSMutableArray*__strong *)texts{
...
//    [[self navigationController] pushViewController:profileSalesChartViewController animated:YES];
    ...
    NSString *beginDate = [NSString stringWithFormat:@"%@",[self getFirstDayOfMonth:[myPicker selectedRowInComponent:0]+1]];
    NSString *endDate = [NSString stringWithFormat:@"%@", [self getLastDayOfMonth:[myPicker selectedRowInComponent:1]+1]];
