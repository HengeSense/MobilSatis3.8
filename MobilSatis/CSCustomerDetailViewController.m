...
- (void)viewSurveyQuestions{
    ...
    CSSurveySelectionViewController *surveyQuestions = [[CSSurveySelectionViewController alloc] initWithUser:user andNibName:nibName andKunnr:myCustomer.kunnr];
...
- (BOOL)checkCustomerDistanceForVisit{
    if ([self getDistanceFromOldPointY:user.location.coordinate.latitude andOldPointX:user.location.coordinate.longitude andNewPointY:myCustomer.locationCoordinate.coordinate.latitude andNewPointX:myCustomer.locationCoordinate.coordinate.longitude]<= 1000) {
        return YES;
...
- (void)checkIn{
    if ([self checkCustomerDistanceForVisit]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Hata" message:@"Ziyaret girişi yapabilmek için noktanın en az 1000 mt. yakınında olmanız gerekmektedir." delegate:nil cancelButtonTitle:@"Tamam" otherButtonTitles: nil];
        [alert show];
    }
...
- (UITableViewCell *)dealerTableView:(int)row andCell:(UITableViewCell *)cell {
    ...
    switch (row) {
        ...
        case 1:
            ...
            break;
        case 2:
            cell.textLabel.text = @"Müşteri Anketi";
            cell.detailTextLabel.text = @"";
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
            [[cell detailTextLabel] setFont:[UIFont systemFontOfSize:[UIFont smallSystemFontSize]]];
            break;
...
//            }
    ...
    if (row == 31 && !([customerDetail.efesContract isEqualToString:@"Analiz yok"])) {
...
- (void)viewDidLoad
{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        sesGroup.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        customerFeature.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        customerPosition.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        customerGroup.frame = 	CGRectMake(0, 525, 700, 180);
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        locationGroup.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        cityPickerList.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
    ...
    if([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad) {
        countyList.frame = 	CGRectMake(0, 525, 700, 180);   
    } 
    else{
