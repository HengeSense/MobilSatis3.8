...
@synthesize counter;
@synthesize showMyCustomer,mapView;
...
- (id)initWithUser:(CSUser *)myUser{
    ...
    {
        self = [super initWithNibName:@"CSLoctionHandlerViewController_Ipad" bundle:nil];
    }
...
- (void)getCustomersFromSapWithLocation:(CSMapPoint*)location{
    ...
    NSString *tableName2 = [NSString stringWithFormat:@"T_TEMSILCI"];
    ...
    [sapHandler addTableWithName:tableName2 andColumns:columns2];
...
-(void)getResponseWithString:(NSString *)myResponse andSender:(ABHSAPHandler *)me{
    ...
    else
    {
        ...
        NSMutableArray *responses2 = [ABHXMLHelper getValuesWithTag:@"ZMOB_S_GET_TEMSILCI" fromEnvelope:myResponse];
        ...
        {
            NSString *salesRepresentativeTable = [NSString stringWithFormat:@"%@",[responses2 objectAtIndex:0]];
            
            [self checkExistingCoreDataToDelete];
            [self initSalesRepresentativeFromResponse:salesRepresentativeTable];
            
            for(CSSalesRepresentative *tempSalesRepresentative in customers){
...
- (void)initCustomersFromResponse:(NSString*)myResponse {
    ...
    survey = [ABHXMLHelper getValuesWithTag:@"SURVEY" fromEnvelope:myResponse];
    ...
    for (int sayac = 0; sayac<[customerNumbers count]; sayac++) {
        ...
        [tempCustomer setSrvy:[survey objectAtIndex:sayac]];
...
- (NSString*) getAnnotationPngNameFromCustomer:(CSCustomer*)activeCustomer{
    if ([activeCustomer.other isEqualToString:@"03"]) {
        ...
        if ([activeCustomer.srvy isEqualToString:@"X"]) {
            activeCustomer.locationCoordinate.tablePngName =@"surveyKapaliBig.png";
            return @"SurveyKapaliSmall.png";
        }
        else
        {
    ...
    if ([activeCustomer.relationship isEqualToString:@"04"] || [activeCustomer.relationship isEqualToString:@"05"]) {
        if ([activeCustomer.hasPlan isEqualToString:@"X"]) {
            if ([activeCustomer.srvy isEqualToString:@"X"])
            {
                activeCustomer.locationCoordinate.tablePngName =@"surveyEvBig.png";
                return @"surveyEvSmall.png";
            }
            else
            {
    ...
    if ([activeCustomer.type isEqualToString:@"acik"]) {
        if ([activeCustomer.hasPlan isEqualToString:@"X"]) {
            ...
            if ([activeCustomer.srvy isEqualToString:@"X"]) {
                activeCustomer.locationCoordinate.tablePngName = @"SurveyAcikBig.png";
                return @"surveyAcikSmall.png";
            }
            else
            {
        ...
        if ([activeCustomer.hasPlan isEqualToString:@"X"]) {
            if ([activeCustomer.srvy isEqualToString:@"X"]) {
                activeCustomer.locationCoordinate.tablePngName = @"surveyEkominiBig.png";
                return @"surveyEkominiSmall.png";
            }
            else
            {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CSCustomer *tempCustomer;
    CSSalesRepresentative *tempSales;
    if ([[customers objectAtIndex:indexPath.row] isKindOfClass:[CSCustomer class]]) {
        tempCustomer = [customers objectAtIndex:indexPath.row];
    ...
    {
        tempSales = [customers objectAtIndex:indexPath.row];
        
        [[cell textLabel] setText: tempSales.name2 ];
        [[cell detailTextLabel] setText:tempSales.kunnr];
    }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    {
        [[self navigationItem] setTitle:@"Müşterilerim"];
